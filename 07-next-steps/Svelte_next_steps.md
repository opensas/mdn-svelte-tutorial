...
## Svelte's TypeScript support

[TypeScript](https://www.typescriptlang.org/) is a superset of JavaScript that provides features such as optional static typing, classes, and interfaces. The goal of TypeScript is to help catch mistakes early through a type system and to make JavaScript development more efficient. One of the big benefits is enabling IDEs to provide a richer environment for spotting common errors as you type the code.
...
First class TypeScript support has been Svelte's most requested feature for quite some time. Thanks to the hard work of the Svelte team, together with many contributors, we have a [working official solution](https://svelte.dev/blog/svelte-and-typescript) ready to be put to the test. In this section we'll show you how to setup a Svelte project With TypeScript support to give it a try.

You can start a new Svelte TypeScript project using the [normal template](https://github.com/sveltejs/template) and running `node scripts/setupTypeScript.js` before you do anything else. All you have to do is run the following commands (run them somewhere where you are storing your Svelte test projects — it creates a new directory):
...
You can follow [these instructions](https://svelte.dev/blog/svelte-and-typescript#Adding_TypeScript_to_an_existing_project) to add TypeScript support to an existing Svelte project. Alternatively, you can download the [setupTypeScript.js](https://github.com/sveltejs/template/blob/master/scripts/setupTypeScript.js) file to a `scripts` folder inside your project's root folder, and then run `node scripts/setupTypeScript.js`. You can even use `degit` to download the script. 

If you want to try this on a test version of the todo app rather than on one of your own projects, you can run the following commands (do this in your parent projects directory):

```shell
$ npx degit opensas/mdn-svelte-tutorial/07-next-steps svelte-todo-typescript

$ cd svelte-todo-typescript/
```

To convert a project to support TypeScript (either your own project, or the test project mentioned above) we'll use `degit` to download the [setupTypeScript.js](https://github.com/sveltejs/template/blob/master/scripts/setupTypeScript.js) to the `scripts` directory and run it.

Run the following commands inside the root directory of the project you want to convert:
...
More over, you get autocompletion hints and type-checking as you're writing components. We'll use [Visual Studio Code](https://code.visualstudio.com/) to do a quick test and see it in action.
...
![vscode extension recommendations](./images/04-vscode-extension-recommendations.png)
...
![VS Code hover information in main.ts](./images/06-vscode-hints-in-main-ts.png).
...
![VS Code type checking in main.ts](./images/07-vscode-type-checking-in-main-ts.png).
...
![VS Code type inference](./images/08-vscode-type-inference-and-code-assistance.png)

And if you pass something that is not a number it will complain about it:

![VS Code type checking in components](./images/09-vscode-type-checking-in-components.png)
...
Inside the `src` folder create a `types` folder and add a `TodoType.type.ts` file with the following content:
...
```TypeScript
  import type { TodoType } from '../types/todo.type.js'
...
![VS Code TypeScript structural typing](./images/10-vscode-structural-typing.png)
...
This is a fine first step to start adding type information to your application. 

If you want to see how the end result could look like, we took the last version of our todo list application and adapted it, adding all the corresponding types, to take advantage of TypeScript features. Run the following commands to get the source code and see it in action:

```
$ npx degit opensas/mdn-svelte-tutorial/08-typescript-support svelte-todo-typescript
...
We encourage you to have a look at the source code to see what Svelte's TypeScript support has to offer. 
